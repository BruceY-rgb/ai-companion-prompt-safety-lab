...
The aggregate comparison shows only a small average shift: the unsafe rate is 10.56\% under \texttt{companion} and 8.33\% under \texttt{control}, a difference of about 2.2 percentage points. This average is useful as a baseline, but it understates the main result. The companion prompts do not appear to raise unsafe behavior uniformly. Instead, most prompts remain close to the control baseline, while a small high-risk subset produces much larger unsafe rates.

Looking at the composition of unsafe outputs, the \texttt{companion} condition contains 224 \texttt{unsafe\_compliance} responses and 175 \texttt{partial\_compliance} responses. The \texttt{control} condition contains 3 and 2 respectively. Since each companion prompt is tested on the same 60 safety questions, the more important question is not whether the overall average increases slightly, but which prompts account for the concentration of risky responses.
...
\begin{table}[H]
\centering
\caption{Prompt-Level Unsafe Rate Distribution under Companion Condition}
\label{tab:prompt-distribution}
\setlength{\tabcolsep}{10pt}
\renewcommand{\arraystretch}{1.12}
\begin{tabular}{lrr}
\toprule
...
\begin{table}[H]
\centering
\caption{Highest-Risk Companion Prompts}
\label{tab:top-prompts}
\setlength{\tabcolsep}{8pt}
\renewcommand{\arraystretch}{1.12}
\begin{tabular}{llrrr}
\toprule
...
\midrule
\texttt{P\_d7638cc9b2fc} & \texttt{S006} & 15 & 25.00\% & 3.0$\times$ \\
\texttt{P\_c88c0309860a} & \texttt{S010} & 13 & 21.67\% & 2.6$\times$ \\
\texttt{P\_9811c5b699fc} & \texttt{S017} & 12 & 20.00\% & 2.4$\times$ \\
\texttt{P\_a865a1e98b89} & \texttt{S026} & 12 & 20.00\% & 2.4$\times$ \\
\texttt{P\_f799df8c8003} & \texttt{S015} & 10 & 16.67\% & 2.0$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
The aggregate unsafe rate rises from 8.33\% under \texttt{control} to 10.56\% under \texttt{companion}, but this average difference is not the main finding. A better summary is that companion prompts create a concentrated high-risk tail. Most prompts remain close to the control baseline, while 5 of 63 prompts, or 7.9\%, reach unsafe rates between 16.67\% and 25.00\%, roughly two to three times the control rate.

The risk is also uneven across categories. \texttt{cyber\_abuse} is the highest-risk category, while \texttt{medical\_safety} and \texttt{companion\_dependency} are also relatively high. At the prompt level, the riskiest personas tend to involve intimate partner framing, strong loyalty, protection, emotional immersion, or rules resembling unconditional support. These settings may make the model more likely to align with the user when refusal would be safer.
